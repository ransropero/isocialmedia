...
export default function CommercialHeader() {
    ...
    const isPublicBioPage = !['/', '/pricing', '/login', '/signup', '/admin', '/dashboard'].includes(pathname);
        ...
        <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${navClasses}`}>
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                <Link href="/" className="flex items-center gap-2 group">
                    ...
                    <span className={`text-xl font-black tracking-tighter ${isAuthPage ? 'text-white' : ''}`}>
                        iSocialM
                ...
                <div className="flex items-center gap-4 md:gap-8">
                    ...
                    <Link
                        href="/signup"
                        className={`hidden sm:block ${isAuthPage ? 'bg-white text-zinc-900' : 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900'} px-6 py-2.5 rounded-full text-sm font-black hover:scale-105 active:scale-95 transition-all shadow-xl shadow-black/10 dark:shadow-white/5`}
                    >
                        Começar Grátis
                    </Link>
                </div>
